...
exports.Surge = exports.UnsignedPriceUpdate = exports.SurgeUpdate = void 0;
const anchor_utils_js_1 = require("../anchor-utils/anchor-utils.js");
const constants_js_1 = require("../constants.js");
const ed25519_instruction_utils_js_1 = require("../instruction-utils/ed25519-instruction-utils.js");
const index_js_1 = require("../utils/index.js");
const signatureAuth_js_1 = require("../utils/signatureAuth.js");
const oracleQuote_js_1 = require("./oracleQuote.js");
...
class SurgeUpdate {
     ...
     * - isHeartbeat: Whether this update was triggered by heartbeat (vs price change)
     * - swInternalLatencyMs: Internal processing time (broadcast_ts - max(verified_at))
     * - oracleBroadcastToClientMs: Time from oracle broadcast to client receipt
     * - emitLatencyMs: Minimum emit latency across all feeds in the bundle
     *
     * Per-feed metrics:
     * - sourceToOracleMs: Time from exchange to oracle reception (seen_at - event_ts)
     * - emitLatencyMs: Total time from exchange event to oracle broadcast
     *   - For price changes: (seen_at - event_ts) + (broadcast_ts - verified_at)
     *   - For heartbeats: (seen_at - event_ts) + (broadcast_ts - heartbeat_at_ts)
     */
    getLatencyMetrics() {
        var _a, _b, _c;
        const clientReceiveTimeMs = Date.now();
        const broadcastTimeMs = (_a = this.rawResponse.broadcast_ts_ms) !== null && _a !== void 0 ? _a : 0;
        const isHeartbeat = this.rawResponse.triggered_on_price_change === false;
        const heartbeatAtMs = (_b = this.rawResponse.heartbeat_at_ts_ms) !== null && _b !== void 0 ? _b : broadcastTimeMs;
        ...
        const feedValues = (_c = this.rawResponse.feed_values) !== null && _c !== void 0 ? _c : [];
        let swInternalLatencyMs;
        if (isHeartbeat) {
            ...
            const maxVerifiedAt = feedValues.reduce((max, feed) => {
                var _a;
                const verifiedAt = (_a = feed.verified_at) !== null && _a !== void 0 ? _a : 0;
        ...
        const perFeedMetrics = feedValues.map(feed => {
            var _a, _b, _c, _d, _e;
            const eventTs = (_a = feed.event_ts) !== null && _a !== void 0 ? _a : 0;
            const seenAt = (_b = feed.seen_at) !== null && _b !== void 0 ? _b : 0;
            const verifiedAt = (_c = feed.verified_at) !== null && _c !== void 0 ? _c : 0;
            const sourceToOracleMs = seenAt - eventTs;
            // Calculate emit latency based on whether this is a heartbeat or price change
            let emitLatencyMs;
            if (isHeartbeat) {
                emitLatencyMs = sourceToOracleMs + (broadcastTimeMs - heartbeatAtMs);
            }
            else {
                emitLatencyMs = sourceToOracleMs + (broadcastTimeMs - verifiedAt);
            }
            return {
                symbol: (_d = feed.symbol) !== null && _d !== void 0 ? _d : '',
                source: (_e = feed.source) !== null && _e !== void 0 ? _e : '',
                feed_hash: feed.feed_hash,
                sourceToOracleMs,
                emitLatencyMs,
            };
        });
        // Bundle-level emit latency is the minimum across all feeds
        const bundleEmitLatencyMs = perFeedMetrics.length > 0
            ? Math.min(...perFeedMetrics.map(f => f.emitLatencyMs))
            : 0;
        return {
            isHeartbeat,
            swInternalLatencyMs,
            oracleBroadcastToClientMs,
            emitLatencyMs: bundleEmitLatencyMs,
            perFeedMetrics,
        };
    }
    /**
     ...
     */
    toQuoteIx(queuePubkey, payer, instructionIdx = 0) {
        var _a;
        ...
        if (!response.oracle_response.ed25519_enclave_signer) {
            throw new Error('Ed25519 enclave signer not available in oracle response');
        }
        ...
        const ed25519Signature = {
            pubkey: buffer_1.Buffer.from(pubkeyHex, 'hex'), // Ed25519 pubkey (32 bytes)
            signature: buffer_1.Buffer.from(response.oracle_response.signature, 'base64'), // Signature
            message: buffer_1.Buffer.from(response.oracle_response.checksum, 'base64'), // Message is the checksum for Ed25519
            oracleIdx: response.oracle_response.oracle_idx, // Oracle index
        };
        // Build the Ed25519 instruction
        const ed25519Instruction = ed25519_instruction_utils_js_1.Ed25519InstructionUtils.buildEd25519Instruction([ed25519Signature], instructionIdx, response.oracle_response.slot, // recent_slot from oracle response
        0 // version 0 for Ed25519 v0 scheme
        );
        // Get feed hashes from the response
        const feedHashes = ((_a = response.feed_values) === null || _a === void 0 ? void 0 : _a.map(feed => '0x' + feed.feed_hash)) || [];
        ...
        const [oracleAccount, bump] = oracleQuote_js_1.OracleQuote.getCanonicalPubkey(queuePubkey, feedHashes, oracleQuote_js_1.QUOTE_PROGRAM_ID);
        const opcode = 0;
        ...
        const quoteProgramIx = new anchor_31_1.web3.TransactionInstruction({
            ...
            data: buffer_1.Buffer.from([opcode, instructionIdx, bump]), // [opcode, ix_idx, bump]
        });
        return [ed25519Instruction, quoteProgramIx];
    }
    /**
     * Get the source timestamp (deprecated field, use getFeedTimestamps for per-feed accuracy)
     * This returns the most recent exchange timestamp across all feeds in the bundle.
     * @returns Source timestamp in milliseconds
     */
    getSourceTimestamp() {
        return this.rawResponse.source_ts_ms;
    }
    /**
     * Get the seen-at timestamp (deprecated field, use getFeedTimestamps for per-feed accuracy)
     * This returns the most recent oracle reception time across all feeds in the bundle.
     * @returns Seen-at timestamp in milliseconds
     */
    getSeenAtTimestamp() {
        return this.rawResponse.seen_at_ts_ms;
    }
    /**
     * Get the broadcast timestamp - when the oracle decided to broadcast this bundle
     * This is captured after signing is complete and represents when the oracle sent the message.
     * @returns Broadcast timestamp in milliseconds, or undefined if not available
     */
    getBroadcastTimestamp() {
        return this.rawResponse.broadcast_ts_ms;
    }
    /**
     * Get the heartbeat timestamp - when the heartbeat was triggered
     * This is only populated when triggered_on_price_change is false (i.e., for heartbeat updates)
     * @returns Heartbeat timestamp in milliseconds, or undefined if not a heartbeat update
     */
    getHeartbeatTimestamp() {
        return this.rawResponse.heartbeat_at_ts_ms;
    }
    /**
     * Get detailed per-feed timestamps for all feeds in this bundle.
     * Each feed has three timestamps tracking the complete data lifecycle:
     * - event_ts: When the price was generated at the exchange
     * - seen_at: When the oracle received the price data
     * - verified_at: When the oracle verified/processed the price data
     *
     * @returns Array of feed timestamp objects with symbol, source, and timestamps
     */
    getFeedTimestamps() {
        if (!this.rawResponse.feed_values)
            return [];
...
exports.UnsignedPriceUpdate = UnsignedPriceUpdate;
/**
 ...
 * ```
 */
class Surge extends events_1.EventEmitter {
    /**
    ...
    static init(params) {
        var _a;
        const gatewayUrl = typeof params.gateway === 'string'
            ? params.gateway
    ...
    constructor(config) {
        ...
        this.signatureAuth = null; // NEW: Signature auth helper
        ...
        this.crossbar = config.crossbarClient || common_1.CrossbarClient.default();
        // Initialize signature auth if keypair is provided
        if (config.keypair && config.connection) {
            this.signatureAuth = new signatureAuth_js_1.SignatureAuth({
        ...
        return __awaiter(this, void 0, void 0, function* () {
            ...
            try {
                ...
                if (this.config.queue) {
                    this.log('🔧 Using provided queue.program.provider.connection for network detection');
                    ...
                    this.log('🔧 No queue/connection provided, attempting to load from Solana CLI config...');
                    try {
                        const env = yield anchor_utils_js_1.AnchorUtils.loadEnv();
                        connection = env.connection;
                        this.log(`✅ Loaded connection from environment: ${connection.rpcEndpoint}`);
                    }
                    catch (err) {
                        this.log(`⚠️ Could not load from environment (${err instanceof Error ? err.message : 'unknown error'}), will use fallback`);
                    }
                }
                // Detect and set network if we have a connection
                if (connection) {
                    try {
                        const isMainnet = yield (0, index_js_1.isMainnetConnection)(connection);
                        const network = isMainnet
                            ? common_1.CrossbarNetwork.SolanaMainnet
                            : common_1.CrossbarNetwork.SolanaDevnet;
                        this.crossbar.setNetwork(network);
                        this.log(`🌐 Auto-detected network from connection: ${network}`);
                    }
                    catch (err) {
                        this.log(`⚠️ Failed to auto-detect network (${err instanceof Error ? err.message : 'unknown error'}), using default (mainnet)`);
                    }
                }
                    ...
                    const network = this.config.network.includes('mainnet')
                        ...
                        : common_1.CrossbarNetwork.SolanaDevnet;
                    this.crossbar.setNetwork(network);
                    this.log(`🌐 Using configured network string: ${this.config.network} → ${network}`);
                }
                else {
                    this.log('🌐 No network info available, using crossbar default (mainnet)');
                }
                const gateway = yield this.crossbar.fetchGateway();
        ...
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (!this.config.crossbarMode && feedHints && feedHints.length > 0) {
                ...
                const feeds = feedHints
                    ...
                    const sourceStr = source_js_1.Source.toString(feed.source || source_js_1.Source.AUTO);
                    return {
                        symbol: feed.symbol,
                        source: sourceStr,
                    };
            ...
            if (this.config.crossbarMode) {
                ...
                try {
                    ...
                    const ipResponse = yield axios_1.default.get('https://api.ipify.org?format=json', { timeout: 2000 });
            ...
            if (this.signatureAuth) {
                ...
                const authHeaders = yield this.signatureAuth.getAuthHeaders();
                if (authHeaders) {
                    Object.assign(headers, authHeaders);
                    this.log(`✅ Generated signature for pubkey: ${(_a = this.config.keypair) === null || _a === void 0 ? void 0 : _a.publicKey.toBase58()}`);
                }
                else {
                ...
                throw new Error('Either apiKey or keypair (with connection) must be provided');
            }
            const response = yield axios_1.default.post(requestUrl, requestBody, { headers });
            ...
            this.log(`✅ Session obtained: ${(_b = sessionData.session_token) === null || _b === void 0 ? void 0 : _b.substring(0, 8)}...`);
            return sessionData;
        });
    }
    /**
            ...
            try {
                ...
                if (this.config.crossbarMode) {
                    ...
                    if (this.signatureAuth) {
                        ...
                        yield this.signatureAuth.refresh();
                        const authHeaders = yield this.signatureAuth.getAuthHeaders();
                        if (authHeaders) {
                            Object.assign(wsHeaders, authHeaders);
                            this.log('🔐 Added fresh signature headers to WebSocket connection');
                        }
                        else {
                            this.log('⚠️ Failed to get signature headers for WebSocket');
                        }
                    }
                    this.ws = new isomorphic_ws_1.default(session.ws_url, {
                        headers: wsHeaders,
                    });
                }
                            ...
                            for (const bundle of subscribedMsg.feed_bundles) {
                                if (bundle.feed_bundle_id) {
                                    ...
                                    const feeds = bundle.feeds.map(f => {
                                        ...
                                        return {
                                            symbol: symbolStr,
                                            source: source_js_1.Source.fromString(f.source),
                                        };
                                    });
        ...
        return __awaiter(this, void 0, void 0, function* () {
                ...
                if ('feedHash' in feed) {
                    ...
                    const normalizedSource = feed.source
                        ? typeof feed.source === 'string'
                            ? source_js_1.Source.fromString(feed.source)
                            : feed.source
                        : source_js_1.Source.WEIGHTED;
                    processedFeeds.push({
                        symbol: feed.symbol,
            ...
            if (batchIntervalMs !== undefined) {
                if (batchIntervalMs < 5 || batchIntervalMs > 10000) {
                    throw new Error('batchIntervalMs must be between 5 and 10000 milliseconds');
                }
            }
            ...
            for (const feed of processedFeeds) {
                const source = source_js_1.Source.toString(feed.source);
            ...
            const signatureFields = yield this.getMessageSignatureFields();
            // Send subscription message (following bundle_verbose_test.ts format)
            const subscriptionMessage = Object.assign({ type: 'Subscribe', feed_bundles: [
                    {
                        feeds: processedFeeds.map(feed => ({
                            symbol: stringToPair(feed.symbol), // Convert string to Pair object
                            source: source_js_1.Source.toString(feed.source || source_js_1.Source.WEIGHTED), // Convert to string
                        })),
                    },
                ], 
                // Default to Ed25519 if not specified in config
                signature_scheme: this.config.signatureScheme === 'secp256k1' ? 'Secp256k1' : 'Ed25519' }, signatureFields);
    ...
    handleSignedPing() {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const sigFields = yield this.getMessageSignatureFields();
            if (!sigFields) {
            ...
            try {
                ...
                for (const feed of feeds) {
                    if ('symbol' in feed) {
                        ...
                        if (feed.source) {
                            const source = typeof feed.source === 'string'
                                ? feed.source
                                : source_js_1.Source.toString(feed.source);
                        ...
                        this.log(`✅ Validated: ${feed.symbol} ${feed.source ? `(${typeof feed.source === 'string' ? feed.source : source_js_1.Source.toString(feed.source)})` : ''}`);
                    }
                }
            }
            catch (error) {
    ...
    subscribeToAll(sources_, batchIntervalMs) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if ((sources_ !== null && sources_ !== void 0 ? sources_ : []).length === 0) {
                sources_ = [source_js_1.Source.WEIGHTED];
            }
            const sources = sources_ === null || sources_ === void 0 ? void 0 : sources_.map(s => source_js_1.Source.toString(s));
            ...
            const subscriptionMessage = Object.assign({ type: 'Subscribe', feed_bundles: [
                    {
                        feeds: processedFeeds.map(feed => ({
                            symbol: stringToPair(feed.symbol), // Convert string to Pair object
                            source: source_js_1.Source.toString(feed.source), // Convert enum to string
                        })),
                    },
                ], 
                // Default to Ed25519 if not specified in config
                ...
                for (const feed of symbolGroup.feeds) {
                    ...
                    if (feed.feed_id === feedHash) {
                        return {
                            ...
                            source: source_js_1.Source.fromString(feed.source),
                        };
                    }
                }
            }
    ...
    getTotalFeeds(endpoint) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                const response = yield axios_1.default.get(`${endpoint}?limit=1`);
    ...
    checkFeedExists(symbol, source) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                ...
                const sourceStr = typeof source === 'string' ? source : source_js_1.Source.toString(source);
    ...
    getFeedInfo(symbol) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            try {
                ...
                const response = yield axios_1.default.get(`${endpoint}?symbol=${symbol.toUpperCase()}`);
        ...
        return __awaiter(this, void 0, void 0, function* () {
            ...
            for (const feed of feeds) {
                if ('feedHash' in feed) {
                    ...
                    const source = source_js_1.Source.toString(feed.source || source_js_1.Source.WEIGHTED);
                    const key = `${source}:${feed.symbol}`;
                    this.subscriptions.delete(key);
                    ...
                    const feedBundles = [
                        {
                            feeds: symbolFeeds.map(feed => ({
                                ...
                                source: source_js_1.Source.toString(feed.source || source_js_1.Source.WEIGHTED),
                            })),
                        },
                    ];
                    unsubscribeMessage = Object.assign({ type: 'Unsubscribe', feed_bundles: feedBundles }, signatureFields);
                }
            }
            else {
                // Oracle mode: use feed_bundle_ids
                if (bundleIds.size > 0) {
                    unsubscribeMessage = Object.assign({ type: 'Unsubscribe', feed_bundle_ids: Array.from(bundleIds) }, signatureFields);
                }
            }
