...
- `open_parametric` 프로그램 아래에는 여러 개의 `MasterPolicy`가 존재할 수 있습니다.
- 리더사(`leader`)가 `MasterPolicy`를 생성합니다.
- `MasterPolicy` 안에는 참여 보험사 목록(`participants`)과 재보험사(`reinsurer`)가 들어갑니다.
- 참여사 지분 합은 10000bps여야 하고, 참여사 목록에는 반드시 `leader`가 포함되어야 합니다.
- 참여사/재보험사의 확인 과정을 거쳐 `MasterPolicy`가 `Active`가 됩니다.
- 이후 개별 보험 가입 건은 `FlightPolicy`로 생성됩니다.

관련 코드:

- [create_master_policy.rs](/Users/deaver/Desktop/Repo/riskmesh/contract/programs/open_parametric/src/instructions/create_master_policy.rs)
...
### "리더사와 원수사/재보험사가 지분을 나누면 MasterPolicy가 구성된다"는 표현은 맞나

대체로 맞지만, 조금 더 정확히 말하면:

- `MasterPolicy`는 리더사가 생성한다.
...
까지 포함해야 실제 운영 가능한 `MasterPolicy`가 됩니다.

## 2. 현재 구조에서 가능한 가입 흐름

현재 컨트랙트가 허용하는 가입 흐름은 아래와 같습니다.

1. 리더사가 `MasterPolicy`를 생성한다.
2. 참여사와 재보험사가 확인을 마쳐 `MasterPolicy`를 `Active`로 만든다.
...
"리더사 시스템이 가입 완료 이벤트를 보내면, 백엔드가 이를 검증한 뒤 해당 `MasterPolicy` 아래에 `FlightPolicy`를 생성한다."

추천 endpoint 예시:

`POST /api/master-policies/:master_pubkey/flight-policies`
...
1. 호출 주체 인증
2. `master_pubkey` 조회
3. 요청자가 해당 `MasterPolicy`의 리더사인지 확인
4. `MasterPolicy.status == Active` 확인
...
현재 `MasterPolicy`에는 이미 `currency_mint`가 있습니다.
...
- 리더사별 mint를 만들고
- 각 `MasterPolicy.currency_mint`에 그 mint를 연결하는

구조는 가능합니다.

다만 이 경우에도 주의할 점이 있습니다.

- 같은 리더사가 여러 `MasterPolicy`를 만들 수 있다.
- 한 리더사가 꼭 하나의 토큰만 가져야 하는 것은 아니다.
- "리더사 = 토큰"으로 고정 매핑하면 나중에 상품 확장이 어려워질 수 있다.

추천:

- 토큰은 리더사 기준보다 `MasterPolicy.currency_mint` 기준으로 보는 것이 더 자연스럽다.
...
## 7. `MasterPolicy`와 리더사 매핑은 어떻게 봐야 하나

질문의 핵심인 "그러면 `MasterPolicy`랑 매핑된 값은 `leader pubkey`가 맞아?"에 대한 답은:

"부분적으로 맞지만, 유일 식별자는 아니다" 입니다.

### 코드 기준 사실

`MasterPolicy` PDA 시드는 다음입니다.

- `["master_policy", leader, master_id]`

관련 코드:

- [create_master_policy.rs](/Users/deaver/Desktop/Repo/riskmesh/contract/programs/open_parametric/src/instructions/create_master_policy.rs#L19)
- [pda.rs](/Users/deaver/Desktop/Repo/riskmesh/backend/src/solana/pda.rs#L29)

즉:

- 하나의 `leader pubkey` 아래 여러 `MasterPolicy`가 있을 수 있습니다.
- 같은 `leader`가 `master_id`만 바꿔 여러 상품을 만들 수 있습니다.
- 반대로 다른 `leader`가 같은 `master_id`를 써도 별개 `MasterPolicy`가 됩니다.

따라서:

- "`leader pubkey`는 어떤 리더사가 소유한 마스터 정책인지 알려주는 소유자 키"로는 맞다
- "하지만 `MasterPolicy` 자체를 유일하게 식별하는 값"으로는 부족하다
...
- 개별 마스터 정책 식별: `master_policy pubkey`가 기준
...
- `leader pubkey` 하나만으로 개별 `MasterPolicy`를 식별하려는 것

## 9. 추천 구현 방향

1. 1차 버전은 "리더사 인증 토큰 + `master_pubkey` 지정" 모델로 간다.
2. endpoint는 `POST /api/master-policies/:master_pubkey/flight-policies` 형태로 만든다.
3. 서버는 인증된 리더사가 해당 `MasterPolicy.leader`와 일치하는지 검증한다.
...
- 개별 상품 식별은 `master_policy pubkey`
