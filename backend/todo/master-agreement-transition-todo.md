...
- `backend/src/oracle/program_accounts.rs`
  - Anchor account discriminator 문자열 `"MasterAgreement"` 유지
  - 이유: on-chain account 이름 `MasterAgreement`와 직접 연결됨
...
- `backend/src/solana/pda.rs`
  - PDA seed literal `b"master_agreement"` 유지
...
- `backend/src/api/router.rs`
  - route path `/api/master-agreements/...` 유지
  - 이유: frontend가 기존 API path를 사용 중일 가능성이 높음

- `backend/src/events.rs`
  - SSE event name `master_agreement_updated` 유지
  - 이유: frontend event listener와의 호환성 유지 필요

- `backend/src/api/service.rs`
  - DB test 응답 키 `master_agreement_count` 유지
  - 이유: 외부 소비 응답 필드 호환성 유지

- `backend/src/api/types.rs`
  - 응답 필드 `master_agreement_pubkeys` 유지
  - 응답 필드 `master_agreement_pubkey` 유지
  - 이유: frontend 응답 파싱 코드와의 호환성 유지

### 저장 데이터 / 마이그레이션

- `backend/src/db.rs`
  - SQLite collection name `master_agreements` 유지
  - 이유: 기존 저장 데이터와 마이그레이션 비용 고려

- `backend/src/firebase/mod.rs`
  - Firestore `kind: "master_agreement"` 유지
  - Firestore field `master_agreement_count` 유지
...
- on-chain account 이름은 여전히 `MasterAgreement`다.
  - `contract/programs/open_parametric/src/state.rs`

- master PDA seed는 여전히 `b"master_agreement"`다.
  - `contract/programs/open_parametric/src/instructions/create_master_agreement.rs`

- flight PDA seed는 여전히 `master_agreement.key()`를 부모로 사용한다.
...
- backend의 account parsing도 contract account 이름에 맞춰 `"MasterAgreement"` discriminator를 그대로 사용한다.
...
- `/api/master-agreements` 계열 route를 `/api/master-agreements`로 바꿀지 결정
- 응답 필드 `master_agreement_pubkey`, `master_agreement_pubkeys`, `master_agreement_count` rename 여부 결정
- SSE event name `master_agreement_updated` rename 여부 결정

### data migration 작업 시

- SQLite collection `master_agreements` rename 여부 결정
- Firestore `kind=master_agreement` 및 `master_agreement_count` 필드 migration 여부 결정

### smart contract 작업 시

- `MasterAgreement` account 명칭 자체를 `MasterAgreement`로 변경할지 결정
- PDA seed `b"master_agreement"` 변경 여부 결정
