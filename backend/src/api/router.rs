...
pub(super) fn build_router(state: AppState) -> Router {
    Router::new()
        ...
        .route(
            "/api/master-policies/accounts",
            get(get_master_agreement_accounts),
        )
        .route(
            "/api/master-policies/:master_policy_pubkey",
            get(get_master_agreement),
        )
        .route(
            "/api/master-policies/:master_policy_pubkey/display-names",
        ...
        .route(
            "/api/flight-policies/:flight_policy_pubkey",
            get(get_flight_policy),
        )
