...
11. [Master/Flight 플로우 (Track A)](#11-masterflight-플로우-track-a)
...
| `MAX_MASTER_PARTICIPANTS` | 8 | 마스터 계약당 최대 참여사 수 |
| `MAX_SUBSCRIBER_REF_LEN` | 64 | 가입자 참조 문자열 최대 길이 |
...
| `MathOverflow` | 6014 | 산술 오버플로우 | checked_mul/sub/add 실패 |
| `MasterNotActive` | 6015 | 마스터 계약 비활성 | MasterPolicy.status ≠ Active |
| `MasterNotConfirmed` | 6016 | 마스터 확인 미완료 | 참여사/재보험사 미확인 |
| `InvalidRole` | 6017 | 잘못된 역할 | confirm_master에 잘못된 role 전달 |
| `InvalidPayout` | 6018 | 잘못된 지급액 | payout_amount == 0 |
| `AlreadySettled` | 6019 | 이미 정산 완료 | premium_distributed == true |
| `InvalidSettlementTarget` | 6020 | 잘못된 정산 대상 | 지갑 소유자 불일치 |
| `InvalidAccountList` | 6021 | 잘못된 계정 목록 | remaining_accounts 수 불일치 |
...
## 11. Master/Flight 플로우 (Track A)

기존 Legacy 플로우(섹션 4)에 추가로 마스터 계약 + 개별 항공 보험 구조가 구현되어 있습니다. 이 섹션은 Track A (Trusted Resolver) 방식의 인스트럭션, 계정, 상태 머신을 설명합니다.

### 11.1 계정 구조

#### MasterPolicy (마스터 계약)

기간 단위로 운영되는 상위 계약입니다. 공통 약관, 지분, 출재율, 지급 티어, 참여사/재보험사 정보를 관리합니다.

```
┌─────────────────────────────────────────────────┐
│ MasterPolicy (4096 bytes)                       │
├─────────────────────────────────────────────────┤
│ master_id             : u64       — 계약 식별자  │
│ leader                : Pubkey    — 리더사 주소   │
│ operator              : Pubkey    — 오퍼레이터    │
│ currency_mint         : Pubkey    — SPL 토큰 Mint│
│ coverage_start_ts     : i64       — 보장 시작     │
│ coverage_end_ts       : i64       — 보장 종료     │
│ premium_per_policy    : u64       — 건당 보험료   │
│ payout_delay_2h       : u64       — 2시간 지연 지급│
│ payout_delay_3h       : u64       — 3시간 지연 지급│
│ payout_delay_4to5h    : u64       — 4-5시간 지급  │
│ payout_delay_6h_or_cancelled : u64 — 6h+/결항 지급│
│ ceded_ratio_bps       : u16       — 출재율 (BPS)  │
│ reins_commission_bps  : u16       — 재보험 수수료  │
│ reinsurer_effective_bps : u16     — 재보험 실효 비율│
│ reinsurer             : Pubkey    — 재보험사 주소  │
│ reinsurer_confirmed   : bool      — 재보험 확인   │
│ reinsurer_pool_wallet : Pubkey    — 재보험 풀 지갑 │
│ reinsurer_deposit_wallet : Pubkey — 재보험 입금 지갑│
│ leader_deposit_wallet : Pubkey    — 리더 입금 지갑 │
│ participants          : Vec<MasterParticipant> (최대 8)│
│ status                : u8        — 계약 상태     │
│ created_at            : i64       — 생성 시각     │
│ bump                  : u8        — PDA bump seed │
└─────────────────────────────────────────────────┘
...
┌─────────────────────────────────────────────────┐
│ MasterParticipant (각 참여사)                     │
├─────────────────────────────────────────────────┤
│ insurer        : Pubkey    — 참여사 주소          │
│ share_bps      : u16       — 지분 비율 (BPS)     │
│ confirmed      : bool      — 확인 여부            │
│ pool_wallet    : Pubkey    — 풀 지갑 (보험금 원천) │
│ deposit_wallet : Pubkey    — 입금 지갑 (프리미엄)  │
└─────────────────────────────────────────────────┘
```

**재보험 실효 비율**: `reinsurer_effective_bps = ceded_ratio_bps * (10000 - reins_commission_bps) / 10000`

#### FlightPolicy (개별 항공 보험)

마스터 계약 하위의 개별 보험 건입니다. 항공편/가입자 정보와 오라클 결과, 지급 상태를 관리합니다.

```
┌─────────────────────────────────────────────────┐
│ FlightPolicy (1024 bytes)                       │
├─────────────────────────────────────────────────┤
│ child_policy_id   : u64       — 개별 보험 식별자  │
│ master            : Pubkey    — 상위 MasterPolicy│
│ creator           : Pubkey    — 생성자 주소       │
│ subscriber_ref    : String    — 가입자 참조 (≤64) │
│ flight_no         : String    — 항공편 번호       │
│ route             : String    — 노선              │
│ departure_ts      : i64       — 출발 예정 시각     │
│ premium_paid      : u64       — 납입 보험료       │
│ delay_minutes     : u16       — 지연 시간 (분)    │
│ cancelled         : bool      — 결항 여부          │
│ payout_amount     : u64       — 지급액             │
│ status            : u8        — 현재 상태          │
│ premium_distributed : bool    — 프리미엄 정산 여부 │
│ created_at        : i64       — 생성 시각          │
│ updated_at        : i64       — 최종 수정 시각     │
│ bump              : u8        — PDA bump seed     │
└─────────────────────────────────────────────────┘
```
...
|----|------|------|
| 0 | `Draft` | 초기 생성 상태 |
| 1 | `PendingConfirm` | 참여사/재보험사 확인 대기 |
| 2 | `Active` | 활성 — 개별 보험 생성 가능 |
| 3 | `Closed` | 종료 |
| 4 | `Cancelled` | 취소 |
...
|----|------|------|
| 0 | `Issued` | 발행됨 |
| 1 | `AwaitingOracle` | 오라클 결과 대기 |
| 2 | `Claimable` | 지급 조건 충족 |
| 3 | `Paid` | 보험금 지급 완료 |
| 4 | `NoClaim` | 지급 조건 미충족 |
| 5 | `Expired` | 프리미엄 정산 완료 (NoClaim → Expired) |

#### 티어드 지급 구조

`resolve_flight_delay`에서 지연 시간에 따라 MasterPolicy의 4단계 지급액을 적용합니다.

| 조건 | 지급 필드 |
|------|----------|
| 결항 또는 지연 ≥ 360분 | `payout_delay_6h_or_cancelled` |
| 240분 ≤ 지연 < 360분 | `payout_delay_4to5h` |
| 180분 ≤ 지연 < 240분 | `payout_delay_3h` |
| 120분 ≤ 지연 < 180분 | `payout_delay_2h` |
| 지연 < 120분 | 0 (지급 없음) |

### 11.3 인스트럭션 상세

#### 11.3.1 `create_master_policy` — 마스터 계약 생성

리더사가 기간 단위 마스터 보험 계약을 생성합니다.

**서명자**: `leader`

**파라미터** (`CreateMasterPolicyParams`):

| 필드 | 타입 | 설명 |
|------|------|------|
| `master_id` | u64 | 마스터 계약 식별자 |
| `coverage_start_ts` | i64 | 보장 시작 시각 |
| `coverage_end_ts` | i64 | 보장 종료 시각 |
| `premium_per_policy` | u64 | 건당 보험료 |
| `payout_delay_2h` | u64 | 2시간 지연 지급액 |
| `payout_delay_3h` | u64 | 3시간 지연 지급액 |
| `payout_delay_4to5h` | u64 | 4-5시간 지연 지급액 |
| `payout_delay_6h_or_cancelled` | u64 | 6시간+/결항 지급액 |
| `ceded_ratio_bps` | u16 | 출재율 (BPS) |
| `reins_commission_bps` | u16 | 재보험 수수료 (BPS) |
| `participants` | Vec\<MasterParticipantInit\> | 참여사 목록 및 지분 |

**검증 규칙**:
- `coverage_start_ts < coverage_end_ts`
- `premium_per_policy > 0`
- 참여사 수 1~8명, `share_bps` 합계 = 10000
- 리더가 참여사 목록에 포함되어야 함
- 모든 지갑의 mint가 `currency_mint`과 일치
...
**상태 전이**: → `MasterPolicy.status = PendingConfirm`

---

#### 11.3.2 `register_participant_wallets` — 참여사 지갑 등록

참여사가 자신의 풀 지갑(보험금 원천)과 입금 지갑(프리미엄 수령)을 등록합니다.

**서명자**: `insurer` (해당 참여사)

**전제 조건**: `MasterPolicy.status` ∉ {Active, Closed, Cancelled}

**검증 규칙**:
- 서명자가 participants 목록에 존재
- pool_wallet.mint == currency_mint
- deposit_wallet.mint == currency_mint

**동작**: 해당 참여사의 `pool_wallet`, `deposit_wallet` 필드 기록

---

#### 11.3.3 `confirm_master` — 마스터 계약 확인

참여사 또는 재보험사가 마스터 계약 참여를 확인합니다.

**서명자**: `actor` (참여사 또는 재보험사)
...
**전제 조건**: `MasterPolicy.status == PendingConfirm`

**검증 규칙**:
- 참여사(role=0): 서명자가 participants에 존재, pool_wallet과 deposit_wallet이 등록 완료
- 재보험사(role=1): 서명자가 `MasterPolicy.reinsurer`와 일치

**동작**:
- 참여사: `MasterParticipant.confirmed` → true
- 재보험사: `MasterPolicy.reinsurer_confirmed` → true

---

#### 11.3.4 `activate_master` — 마스터 계약 활성화

오퍼레이터가 모든 참여사/재보험사 확인 완료 후 마스터 계약을 활성화합니다.
...
- 모든 참여사 `confirmed == true` 및 지갑 등록 완료

**동작**: `MasterPolicy.status` → `Active`

---

#### 11.3.5 `create_flight_policy_from_master` — 개별 항공 보험 생성

활성 마스터 계약 하위에 개별 항공편 보험을 생성합니다. 보험료는 생성 시점에 leader_deposit_wallet으로 선납됩니다.

**서명자**: `creator` (leader 또는 operator)

**파라미터** (`CreateFlightPolicyParams`):

| 필드 | 타입 | 설명 |
|------|------|------|
| `child_policy_id` | u64 | 개별 보험 식별자 |
| `subscriber_ref` | String | 가입자 참조 (≤64자) |
| `flight_no` | String | 항공편 번호 (≤16자) |
| `route` | String | 노선 (≤16자) |
| `departure_ts` | i64 | 출발 예정 시각 |

**전제 조건**: `MasterPolicy.status == Active`

**검증 규칙**:
- creator가 leader 또는 operator
- 문자열 길이 제한 준수
- 토큰 계정 mint 일치

**동작**:
1. creator의 토큰 계정 → leader_deposit_wallet으로 `premium_per_policy` SPL 전송
2. FlightPolicy 계정 생성 (PDA: `["flight_policy", master_policy, child_policy_id]`)
3. `FlightPolicy.status` → `AwaitingOracle`

**토큰 흐름**:
```
생성자 토큰 계정 ──(SPL Transfer)──▶ leader_deposit_wallet
```

---

#### 11.3.6 `resolve_flight_delay` — 오라클 지연 해소 (Track A)

리더 또는 오퍼레이터가 AviationStack API에서 가져온 지연 데이터로 FlightPolicy를 해소합니다. Trusted Resolver 방식으로, 서명자의 신뢰에 의존합니다.

**서명자**: `resolver` (leader 또는 operator)
...
**동작**:
1. 티어드 지급 테이블에서 payout 계산
2. payout > 0 → `FlightPolicy.status` → `Claimable`
3. payout == 0 → `FlightPolicy.status` → `NoClaim`
4. `delay_minutes`, `cancelled`, `payout_amount` 기록

---

#### 11.3.7 `settle_flight_claim` — 항공 보험 클레임 정산

Claimable 상태의 FlightPolicy에 대해 보험금을 정산합니다. 각 참여사의 풀 지갑과 재보험사 풀 지갑에서 리더 입금 지갑으로 보험금을 집금합니다.

**서명자**: `executor` (leader 또는 operator)

**전제 조건**:
- `MasterPolicy.status == Active`
- `FlightPolicy.status == Claimable`
- `payout_amount > 0`
- `remaining_accounts` 수 = 참여사 수

**금액 계산**:
- 재보험사 부담액: `payout * reinsurer_effective_bps / 10000`
- 보험사 부담액: `(payout - 재보험사 부담액)` → `share_bps` 비율로 분배

**토큰 흐름**:
```
재보험사 풀 지갑 ──(PDA-signed Transfer)──▶ leader_deposit_wallet
참여사A 풀 지갑  ──(PDA-signed Transfer)──▶ leader_deposit_wallet
참여사B 풀 지갑  ──(PDA-signed Transfer)──▶ leader_deposit_wallet
```

**상태 전이**: `FlightPolicy.status` → `Paid`

---

#### 11.3.8 `settle_flight_no_claim` — 항공 보험 무클레임 프리미엄 정산

NoClaim 상태의 FlightPolicy에 대해 보험료를 참여사/재보험사에게 분배합니다. 리더 입금 지갑에 모인 보험료를 각자의 입금 지갑으로 전송합니다.
...
**금액 계산**:
- 재보험사 몫: `premium * reinsurer_effective_bps / 10000`
- 보험사 몫: `(premium - 재보험사 몫)` → `share_bps` 비율로 분배

**토큰 흐름**:
```
leader_deposit_wallet ──(PDA-signed Transfer)──▶ 재보험사 입금 지갑
leader_deposit_wallet ──(PDA-signed Transfer)──▶ 참여사A 입금 지갑
leader_deposit_wallet ──(PDA-signed Transfer)──▶ 참여사B 입금 지갑
```

**상태 전이**: `FlightPolicy.status` → `Expired`, `premium_distributed` → true
...
| 인스트럭션 | Leader | Operator | Participant | Reinsurer |
|-----------|--------|----------|-------------|-----------|
| `create_master_policy` | **필수** | - | - | - |
| `register_participant_wallets` | - | - | **필수** | - |
| `confirm_master` | - | - | **가능** | **가능** |
| `activate_master` | - | **필수** | - | - |
| `create_flight_policy_from_master` | **가능** | **가능** | - | - |
| `resolve_flight_delay` | **가능** | **가능** | - | - |
| `settle_flight_claim` | **가능** | **가능** | - | - |
| `settle_flight_no_claim` | **가능** | **가능** | - | - |

### 11.6 전체 플로우 시나리오

#### 마스터 계약 설정 → 개별 보험 → 클레임 정산

```
시간 ──────────────────────────────────────────────────────────────▶

 1. create_master_policy     2. register_participant_wallets (x N)
 리더가 마스터 계약 생성      각 참여사가 풀/입금 지갑 등록
 [PendingConfirm]           pool_wallet, deposit_wallet 기록
     │                          │
     ▼                          ▼
 MasterPolicy 생성           지갑 정보 기록

 3. confirm_master (x N)     4. activate_master
 참여사/재보험사 확인          오퍼레이터가 활성화
 confirmed = true            [Active]
     │                          │
     ▼                          ▼
 모든 confirmed 확인         개별 보험 생성 가능

 5. create_flight_policy     6. resolve_flight_delay
 개별 항공 보험 생성          오라클 지연 해소
 premium 선납                [Claimable 또는 NoClaim]
 [AwaitingOracle]
     │                          │
     ▼                          ▼
 FlightPolicy 생성           payout 계산/상태 결정
 보험료 → leader_deposit

 7a. settle_flight_claim     7b. settle_flight_no_claim
 (Claimable인 경우)          (NoClaim인 경우)
 보험금 집금                  프리미엄 분배
 [Paid]                      [Expired]
     │                          │
     ▼                          ▼
 풀 지갑들 → leader_deposit  leader_deposit → 입금 지갑들
```
...
| 계정 | 할당 크기 | 비고 |
|------|----------|------|
| MasterPolicy | 4,096 bytes | MasterParticipant * 8 + 고정 필드 (여유 버퍼) |
| FlightPolicy | 1,024 bytes | 고정 필드 + String(subscriber_ref, flight_no, route) |
